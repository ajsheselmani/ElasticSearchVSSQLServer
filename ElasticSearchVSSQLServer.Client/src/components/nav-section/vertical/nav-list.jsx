...
export function NavList({ data, depth, render, slotProps, checkPermissions, enabledRootRedirect }) {
  ...
  const isActive = isActiveLink(pathname, data.path, !!data.children) || pathname === data.path || pathname.startsWith(data.path + '/') || data.children?.some(child => child.path === pathname || pathname.startsWith(child.path + '/'));
