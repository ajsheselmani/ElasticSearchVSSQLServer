...
public class LogService(IGenericRepository<LogDTO, int> logRepository, ILogger<LogService> logger): ILogService
{
    ...
    public async Task<IEnumerable<LogDTO>> GetAllLogsData()
    {
        logger.LogInformation("Kerkese per marrje te te gjithe llogave te sistemit nga SQLServer");
        var logsData = await logRepository.GetAllAsync();
        logger.LogInformation("Marrja e te gjithe llogave te sistemit. Numri i rekordeve: {Count}", logsData?.Count() ?? 0);
